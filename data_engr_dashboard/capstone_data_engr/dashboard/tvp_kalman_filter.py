...
class KalmanFilter:
    ...
    def _filter_full(self, observations):
        ...
        for t in range(n_obs):
            ...
            innovation = y[t] - (h @ pred_means[t]).item()
            innovation_var = (h @ pred_covs[t] @ h.T + r).item()
        ...
        for _ in range(n_iter):
            ...
            if "observation_covariance" in self.em_vars:
                ...
                for t, obs in enumerate(y):
                    h = self.observation_matrices[t, 0].reshape(1, -1)
                    residual = obs - (h @ smoothed_means[t]).item()
                    residual_vars.append(residual**2 + (h @ smoothed_covs[t] @ h.T).item())
